...
;(function($) {
	...
	function createHelper() {
		...
		helper.parent = $('<div id="tooltip"><h3></h3><div class="body"></div><div class="url"></div></div>')
			// hide it at first
			.hide()
			.css({position: 'absolute', zIndex: 4000})
			// add to document
			.appendTo('body');
	...
	function save(e) {
		...
		if ( ($.Tooltip.blocked || this == current || !this.tooltipText) && !$.isFunction(this.tSettings.bodyHandler) )
			return;
		e.preventDefault();
		e.stopPropagation();
		// save current
		...
		if (this.tSettings.fixPNG && $.ifixpng)
			helper.parent.ifixpng();
