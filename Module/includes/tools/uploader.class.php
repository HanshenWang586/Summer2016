...
// We need a bit longer for uploads
set_time_limit(5);

class UploaderTools extends CMS_Class {
	/**
	...
	public function lastMessage() {
		return array_top($this->messages);
	}
	...
	public function captureUpload($fieldname, $file = false) {
		...
		if (!request($file['name'])) {
			$this->logL(constant('LOG_USER_ERROR'), 'E_UPLOAD_FAILED_NO_FILENAME');
			$this->failed[] = $file;
			return false;
		}
	...
	public function addFile($location, $file = array()){
		if (!$this->folder) {
			$this->logL(constant('LOG_USER_ERROR'), 'E_UPLOAD_FAILED_NO_UPLOAD_FOLDER');
			return false;
		}
		...
		$file['extension'] = $extension = strtolower($pathinfo["extension"]);
		...
		if (
			(is_array(($this->extensions)) && !in_array($extension, $this->extensions)) ||
			(is_array(($this->mimes)) && !in_array($file['type'], $this->mimes))
		) {
		...
		if ($file['isImage'] = in_array($file['extension'], $this->imageExtensions)) {
			if (!$file['imagesize'] = getimagesize($location) or !($file['imagesize'][0] > 0)) {
				$this->logL(constant('LOG_USER_ERROR'), 'E_INVALID_IMAGE_UPLOAD');
				$this->failed[] = $file;
				return false;
			}
		}
