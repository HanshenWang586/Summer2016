<?
class MailerTools extends CMS_Class {
	private $mailer;
	private $prefs;
	...
	function validateEmail($email, $source) {
		...
		if (!$validate) $this->log(constant('LOG_SYSTEM_WARNING'), sprintf("<strong>Mailer:</strong> %s address \"%s\" could not be validated.", $source, $email));
		return $validate;
	}
	...
	function addTo($address, $name = "") {
		...
		if (is_string($address) and strpos($address, ',') !== false) $address = explode(',', $address);
	...
	function addSubject($subject) {
		$this->getMailer()->Subject = replaceVars($subject, $this->replaceList);
	}
	...
	function attachUploads($uploadFolder = false, $options = array()) {
		...
		if ($uploadFolder) {
			...
			$uploader = $this->tool('uploader', $uploadFolder, $options['extensions']);
			if ($uploader->captureAllUploads()) {
				$files = $uploader->successful;
			} else $this->log(constant('LOG_SYSTEM_WARNING'), "<strong>File upload:</strong> " . $uploader->lastMessage());
		} else $files = $_FILES;
	...
	// We can also store emails in ewyse, if we wish!
	function storeEmail($folder) {
		if ($folder_id = $this->tool('folder')->getFolderID($folder, 'emails')) {
			$mailer = $this->getMailer();
			...
			$params = array(
				...
				'to'		=> join(', ', array_transpose($mailer->to, 0)) // Bit cryptic; the mailer->to is an array of indiced arrays, of which $mailer->to[0][0] for instance is the email address
			);
			return typical_new('emails', $params);
		}
		return false;
	}
	...
	function send($options = array()) {
		...
		if (!$mailer->From) {
			...
			$mailer->From
				...
				$this->log(constant('LOG_SYSTEM_WARNING'), "<strong>Mailer:</strong> No Sender email address was given. Cannot send email");
				return false;
			}
		}
		...
		if (!$mailer->to) {
			...
			$mailer->to
				...
				$this->log(constant('LOG_SYSTEM_WARNING'), "<strong>Mailer:</strong> No Receiver email address was given. Cannot send email");
				return false;
			}
		}

		// if a folder is set, we can store the email!
		if ($options['folder']) $this->storeEmail($options['folder']);
		...
		try {
			...
			$this->log(constant('LOG_SYSTEM_ERROR'), sprintf("<strong>Mailer:</strong> Sending email threw exception: \"%s\".", $e));
		}
		if (!$succes) $this->log(constant('LOG_SYSTEM_ERROR'), sprintf("<strong>Mailer:</strong> Sending email generated the error: \"%s\".", $mailer->errorInfo));
		return $succes;
	}
}
