    ...
    */
    protected function buildOutput()
    {
        ...
        foreach ($this->outputArray as $word=>$wordArray) {
            ...
            if (!empty($this->_link)) {
                $this->outputCloud .= '<a href="' . sprintf($this->_link, urlencode($word)).'">';
            } // if (!empty($this->_link))
            
            $this->outputCloud .= '<span class="' . $this->_cssPrefix.$wordArray['score'].'"';
		...
		$this->outputCloud = substr($this->outputCloud, 0, strlen($this->outputCloud) - strlen($this->_spacer));
        return $this;
