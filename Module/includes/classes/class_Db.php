...
class Db {
	...
	public function get_query($table, $clauses = false, $params = array()) {
		...
		elseif ($transpose = request($params['transpose'])) {
			if (is_string($transpose)) $getFields = array($transpose);
			elseif (is_array($params['transpose']) && (!array_key_exists('selectValue', $params['transpose']) || $params['transpose']['selectValue'] !== true)) {
				$getFields = array_values($params['transpose']);
			}
		}
	...
	public function run_select($query, $singleResult = false, $params = array()) {
		...
		if ($this->debug()) $time = microtime_float();
		$result = $this->run_query($query);
		$return = array();
		$transpose = request($params['transpose']);
		if (is_string($transpose)) $selectKey = $transpose;
		else $selectKey = (is_array($transpose) and isset($transpose['selectKey'])) ? $transpose['selectKey'] : false;
		$selectValue = (is_array($transpose) and isset($transpose['selectValue'])) ? $transpose['selectValue'] : false;
		...
		if ($result && $this->num_rows($result) > 0) {
			if ($singleResult) {
				...
				if ($transpose === true) {
					foreach($row as $key => $value)	$groupKey ? $return[$groupKey][$key][] = $value : $return[$key][] = $value;
				} elseif ($transpose and $selectKey and array_key_exists($selectKey, $row)) {
					if ($selectValue) {
					 	if ($selectValue === true) $groupKey ? $return[$groupKey][$row[$selectKey]] = $row : $return[$row[$selectKey]] = $row;
						elseif (isset($row[$selectValue])) $groupKey ? $return[$groupKey][$row[$selectKey]] = $row[$selectValue] : $return[$row[$selectKey]] = $row[$selectValue];
					} elseif(request($row[$selectKey])) $groupKey ? $return[$groupKey][] = $row[$selectKey] : $return[] = $row[$selectKey];
				} else $groupKey ? $return[$groupKey][] = $row : $return[] = $row;
			}
 		}
		
 		if ($this->debug() && strpos($query, 'EXPLAIN') === false) {
			$this->runSelect[] = array(
				...
				'explain' => ($this->options['db_explain'] and strpos($query, 'EXPLAIN') === false) ? $this->run_select('EXPLAIN EXTENDED ' . $query) : false
			);
		}
		return $return;
	}
